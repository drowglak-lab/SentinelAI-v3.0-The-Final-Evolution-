...
async def outbox_relay_worker():
    ...
    while True:
        try:
            ...
            cursor = await db.execute("SELECT id, aggregate_id, event_type FROM outbox_events WHERE status = 'PENDING' LIMIT 10")
            events = await cursor.fetchall()

            for event in events:
                # ЗДЕСЬ ДОЛЖНА БЫТЬ ОТПРАВКА В KAFKA
                print(f"📨 [Kafka Emulator] Publishing event: {event['event_type']} for Payment: {event['aggregate_id']}")
